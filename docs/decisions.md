...
### 2026-09-23: 2020 SAS weight returns to plot-level; 2019 is an outlier
- **Decision:** Accept 2020 as having plot-level weight (`Plot_weight` in production file), matching 2024+ structure. Treat 2019's segment-level weight as a one-year structural anomaly.
- **Reason:** 2020 data inspection confirms `Plot_weight` varies per plot (not constant within segment), same pattern as 2024 and expected for a standard survey design. 2019 was the exception: segment-level weight likely reflects a different sampling methodology that year (segment sampling followed by census of plots). No evidence of a continuing alternation between segment- and plot-level weights.
- **Alternatives considered:** Pool segment-level and plot-level weights by deriving plot-level equivalents for 2019. Rejected: would require assumptions about plot-size proportionality and lose the actual design information; better to document 2019 as structurally different and handle it separately in variance estimation.
- **Reversible:** Yes. If 2021-2023 audits reveal that 2019 was part of a multi-year block with segment weighting, this decision can be revisited to treat 2019 and future segment-weighted years as a separate stratum or to explicitly model the weighting change in the pooled estimator.
- **Impact on pipeline:** 2020-2025 can be pooled directly on weight column; 2019 must be flagged as using a different design (segment weight) in the model card and variance estimation. The clean separation is an advantage: avoids a hybrid estimator that mixes two designs.
