...
66 commits, all still on a single compressed development arc. The full data pipeline now runs end to end on real data: SAS microdata harmonization, yield/QC cleaning, survey-weighted district estimates (validated against a real published NISR report), satellite feature extraction including climatology and lead-time cutoffs, attainable yield and yield gap, a driver model (LightGBM + SHAP) per crop, and a nowcast model with a leave-one-year-out backtest. A round of external review this session found and fixed four real correctness bugs in the nowcast (a satellite date-alignment error, feature leakage, and survey-noise contamination of the training target) and reported the results honestly, including where the models do not beat a naive baseline. 163 tests pass, `ruff`/`mypy` clean. The API has grown to 8 of the ~9 planned endpoints (`/drivers` and `/backtest` found and documented a real gap: neither the driver model nor the nowcast ever split by season; `/meta`, `/kpis`, `/districts/{code}/profile`, `/scenario/{code}` added this session, all mechanical composites over `data/public/`, no new modeling). Only `/nowcast`, `/nowcast/{code}/curve`, `/briefs/{code}.pdf` remain, all genuinely blocked on missing modeling/export work. `make lint && make test` is green across the whole repo: the web app's `tsc`, `eslint`, and `vitest` all pass, and it now has a first real product page (`MapPage.tsx`, a district yield-gap choropleth). `models/scenario.py` now exists, writing `data/public/scenario.csv` (the precomputed lever grid the scenario explorer will read); `export/` still does not. Rough MVP completion: **~70%** — the entire data-to-model pipeline including the scenario grid is real, tested, and honestly validated, and the API surface is nearly complete; what's missing is mostly web app pages, the three blocked endpoints, and the export module.
...
- **`data/public/`**: 8 real files now exist — `district_yield.csv`, `yield_gap.csv`, `district_zones.csv`, `drivers.csv`, `drivers_by_district.csv`, `nowcast_backtest.csv`, `scenario.csv`, and `geo/districts_adm2.geojson`. All pass `scripts/check_public.py`.
- **API** (`api/app/`): `GET /health`, `GET /api/v1/meta`, `GET /api/v1/kpis`, `GET /api/v1/districts` (now also carries the NISR `district_code` alongside `gaul_district_code`, joined server-side from `data/reference/district_crosswalk.csv`, see `docs/decisions.md` 2026-09-24), `GET /api/v1/districts/{code}/profile`, `GET /api/v1/yield-gap`, `GET /api/v1/drivers`, `GET /api/v1/backtest`, `GET /api/v1/scenario/{code}` (crop/season/year filtered where applicable, every row including `"suppressed"` ones returned with a reliability flag rather than silently dropped, per `CLAUDE.md` golden rule 6). 8 of ~9 planned endpoints now implemented; only `/nowcast`, `/nowcast/{code}/curve`, and `/briefs/{code}.pdf` remain, all genuinely blocked on missing modeling/export work rather than unwired. Live-verified with a real `uvicorn` run, not just unit tests. A real `DATA_DIR` semantics bug (the real `.env` config pointed at the wrong subfolder) was found and fixed via this live check.
...
- **API**: 8 of ~9 endpoints (`/health`, `/meta`, `/kpis`, `/districts`, `/districts/{code}/profile`, `/yield-gap`, `/drivers`, `/backtest`, `/scenario/{code}` — that's 9 counting `/health` separately from the build guide's ~9 data endpoints). `/nowcast`, `/nowcast/{code}/curve`, `/briefs/{code}.pdf` are unimplemented, all genuinely blocked: `/nowcast` needs per-district-per-year predictions that `models/nowcast.py` does not currently write anywhere (only aggregate backtest metrics exist); `/nowcast/{code}/curve` needs a per-district NDVI/rainfall time series not currently exported to `data/public/`; `/briefs/{code}.pdf` needs PDF rendering, which does not exist anywhere in the repo.
...
| Module | Status |
|---|---|
| ~~`models/scenario.py` — precomputed lever grid~~ | Done, `data/public/scenario.csv`; `GET /scenario/{code}` endpoint still missing |
...
1. ~~Wire `/drivers` and `/backtest` API endpoints~~ — done. ~~Ship one real end-to-end page~~ — done (yield-gap map). ~~Wire `district_name` into `stg_sas_plot_crop`~~ — done. ~~`models/scenario.py`~~ — done. ~~Wire `/meta`, `/kpis`, `/districts/{code}/profile`, `/scenario/{code}`~~ — done this session.
2. **`/nowcast`, `/nowcast/{code}/curve`, `/briefs/{code}.pdf`**: the three remaining endpoints, all genuinely blocked on missing modeling/export work (see "What is partial or broken" above), not just wiring. *Usability, Tangible impact.* — **M-L**
3. **`export/`**: formalize the public-output step (PDF briefs, per-crop GeoJSON), or make an explicit decision to keep the current `models/run.py`-writes-directly-to-`data/public/` pattern instead. *Tangible impact.* — **M**
